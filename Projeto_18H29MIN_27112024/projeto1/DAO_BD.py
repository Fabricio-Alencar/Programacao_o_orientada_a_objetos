...
class ConexaoBD:
    def __init__(self):
        try:
            ...
            print("Conexão estabelecida com sucesso1!")
...
class GenericDAO(BaseDAO):
    ...
    def visualizar(self, **filtros): #** --> pode receber inumeras variáveis
        try:
            if filtros:
                condicoes = ' AND '.join([f"{coluna} = ?" for coluna in filtros.keys()])
                query = f"SELECT * FROM {self.tabela()} WHERE {condicoes}"
                self.cursor.execute(query, tuple(filtros.values())) #tuple é tipo uma lista mas ela é imutável
            else:
